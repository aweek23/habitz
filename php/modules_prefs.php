...
try {
    ...
    // 2) Canon des modules + ordre par défaut
    $modules = [
        'tasks'    => 1,
        'habits'   => 2,
        'projects' => 3,
    ...
    // ===== helpers =====
    $fetchAll = function() use ($pdo, $uid, $modules){
    ...
    $ensureSeed = function() use ($pdo, $uid, $modules){
        ...
        try {
            foreach ($modules as $k=>$ord) {
                $ins = $pdo->prepare("
                    INSERT INTO `".TABLE_USER_MODULES."` (user_id, module_key, visible, ord)
                    VALUES (:uid, :k, 'Yes', :o)
                ");
